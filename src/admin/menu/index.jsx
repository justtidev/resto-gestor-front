...
function MenuIndex() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    console.log(loading)
    axios.get('/menuItem/').then((respuesta) => {
      setLoading(false);
    ...
    <>
      ...
      <div className='flex items-center justify-between p-6'>
         ...
         <button
                  ...
                  onClick={() => navigate(`/admin/menuItem/nuevo`)}
                >
        ...
        <table className='mx-auto text-center  border-gray-400 table-auto shadow-md'>
          ...
          <tbody className='bg-white '>
            {filtrarElementosSegunPagina().filter((menuItem) =>
              ...
              <tr className='  text-center hover:bg-gray-400 odd:bg-gray-300 transition duration-200 ease-in-out'
                key={menuItem.id}>
               
                <td className='p-3 text-center'>{menuItem.nombre}</td>
               
                <td className='p-3 text-center'>{menuItem.precio_item}</td>
                <td className='p-3 text-center'>{menuItem.Categorium.nombre}</td> 
               {menuItem.disponible ? 
              <td className='p-3 text-center'> <div className="inline-flex items-center gap-3"> <FaCircleCheck className="text-green-600" />  </div> </td>  
               :  <td className="p-3 text-black text-center "> <div className="flex justify-center items-center gap-3"></div> <FaTimes/> </td>}
                

                <td className='flex m-2 gap-3 px-4'>
                  <div
                                        onClick={() => handleEdit(menuItem.id)}
                                        className="inline-flex items-center gap-3 px-3 py-1 text-green-600 cursor-pointer rounded hover:bg-accent"
                                        aria-label="Editar Item"
                                      >
                                        <FaEdit title= "Editar" /> 
                                      </div>
                          {/* ESTO ERA PARA SUBIR LA IMAGEN DESDE EL PRODUCTO PERO NO SE ESTA CARGANDO ASI. SE MANEJA DESDE IMAGEN. */}
                                       <div
                    className="inline-flex items-center gap-1 px-3 py-1 text-blue-600 rounded hover:bg-accent "
                    onClick={() => handleInsImg(menuItem.id)}
                  >
                    <FaCamera/>
          ...
          {filtrarElementosSegunPagina().filter((menuItem) =>
            ...
            <div  >
              {Array.from({ length: calcularCantidadPaginas() }, (_, i) => (
                <button
                  key={i + 1}
                  className={`mx-2 py-2 px-4 rounded ${paginaActual === i + 1 ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-700'
                    }`}
